    ...
    "tables": [
        ...
        # ── Sachet lending platform — MongoDB BSON dumps ──────────────────────
        {
            ...
            "columns": [
                ...
                {"name": "leadstatus",                        "type": "String",            "description": "FINAL OUTCOME / disposition status (DIFFERENT from leadstage which tracks funnel position). Values: inProgress, Lead Verified, PAYMENT_INITIATED, PAYMENT_REJECTED, PROPOSAL_INITIATED, POLICY_ISSUED, Application Not Submitted, DRAFT, any, closed_won, active. Use leadstage for WHERE-in-funnel; use leadstatus for final outcome."},
                ...
                {"name": "tenant",                            "type": "String",            "description": "Tenant / distribution platform: turtlemint, turtlefin, centralbank, flipkart, nsdl, itsEazr, pharmeasy."},
                {"name": "customersegment",                   "type": "String",            "description": "Customer segment code, e.g. A1 (premium / high-value segment)."},
                {"name": "utm_source",                        "type": "String",            "description": "UTM marketing source that originated this lead (e.g. google, facebook, partner, organic)."},
                {"name": "utm_medium",                        "type": "String",            "description": "UTM marketing medium (e.g. cpc, email, referral, organic)."},
                {"name": "utm_campaign",                      "type": "String",            "description": "UTM campaign name for marketing attribution."},
                ...
                {"name": "journey_journeytype",               "type": "String",            "description": "Journey type: assisted (partner-assisted), direct (customer self-serve), or Redirect (customer redirected from external platform like Flipkart/NSDL to complete journey)."},
                ...
                {"name": "leadcustomerinfo_creditinfocrif_totalcreditutilization","type": "Nullable(Float64)", "description": "Credit utilization PERCENTAGE (0-100) from CRIF. Different from totalcreditutilized (which is INR amount). E.g. 27.4 means 27.4% of total credit limit is used."},
                {"name": "leadcustomerinfo_email",            "type": "String",            "description": "Customer email address."},
                ...
                # BRE (Business Rules Engine) fields
                {"name": "brerejectioncode",                  "type": "String",            "description": "BRE (Business Rules Engine) rejection code — numeric/string code indicating which BRE rule caused rejection."},
                {"name": "brerejectionreason",                "type": "String",            "description": "BRE (Business Rules Engine) rejection reason — human-readable reason e.g. 'Credit score below threshold'. BRE runs before the live lender API check."},
                {"name": "bremaxloaneligibility",             "type": "Nullable(Float64)", "description": "Maximum loan amount (INR) the BRE system calculated this applicant is eligible for."},
                # KYC and retry fields
                {"name": "kycretrycount",                     "type": "Nullable(Int32)",   "description": "Number of times the customer retried KYC (Know Your Customer) verification."},
        ...
        },
        {
            ...
            "columns": [
                ...
                {"name": "partnerapplicationid",              "type": "String",            "description": "Lender-assigned application ID — generated by the specific lender's system (e.g. SMFGPL025300 for SMFG). Different from Turtlemint's own referenceid."},
